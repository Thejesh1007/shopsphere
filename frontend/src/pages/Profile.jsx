      ...
      <div style={{ maxWidth: 500 }}>
        ...
        <div className="card" style={{ marginBottom: '1rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
            ...
            <div>
              <p style={{ fontWeight: 700, fontSize: '1.1rem', marginBottom: '2px' }}>{user?.name}</p>
              <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>{user?.email}</p>
            </div>
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.65rem 0.9rem', background: 'var(--bg-secondary)', borderRadius: 8 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.65rem 0.9rem', background: 'var(--bg-secondary)', borderRadius: 8 }}>
              <span style={{ color: 'var(--text-secondary)', fontSize: '0.9rem' }}>Total Orders</span>
            ...
            {memberSince && (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.65rem 0.9rem', background: 'var(--bg-secondary)', borderRadius: 8 }}>
                ...
                <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>{memberSince}</span>
              </div>
            )}
          </div>
        </div>
