...
export default function Products() {
  ...
  const handleSearch = (e) => { e.preventDefault(); setSearchParams({}); fetchProducts() }
  ...
  return (
    <div className="page">
      ...
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <select className="input" style={{ width: 'auto', minWidth: 160 }} value={categoryId}
          ...
          {pagination.totalPages > 1 && (
            <div style={{ display: 'flex', justifyContent: 'center', gap: '0.5rem', marginTop: '2rem' }}>
              {Array.from({ length: pagination.totalPages }, (_, i) => i + 1).map(p => (
                <button key={p} className={`btn ${p === page ? 'btn-primary' : 'btn-secondary'} btn-sm`}
